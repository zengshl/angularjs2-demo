...
export interface IPersonIdValidationConfig {
  required: boolean,
  //maxLength: number,
  pattern: RegExp;
}
 ...
 */
export class PersonIdValidation extends BaseValidation<IPersonIdValidationConfig> implements IFieldValidation {

  // accept a configuration. any unspecified values will be defaulted based on getDefaultConfig()
  constructor(config?: IPersonIdValidationConfig) {
    super(config);
  }
    ...
    return (control: AbstractControl): IFieldValidatorResult => {
      ...
      if (HasValue.check(control) == false) {
        if (this.config.required) {
          result.addMessage('required', `身份证号不能为空！`, this.config);
        }
      }
      else {
        ...
        if (IsPattern.check(control, this.config.pattern) === false) {
          result.addMessage('isPersonId', `身份证号格式不正确！`, this.config);
        }
      }
      //}
  ...
  public getDefaultConfig(): IPersonIdValidationConfig {
    return PersonIdValidation.defaultConfig;
  }

  // static access to a default configuration
  public static get defaultConfig(): IPersonIdValidationConfig {
    return {
      required: true,
